...
# Skip this file as it is from the carla examples
# We moved our CameraManager to a separate file
# pylint: skip-file
"""
...
class World:
    def __init__(self, carla_world, hud, args):
        ...
        self._attach_ego_vehicle = args.attach_ego_vehicle
        self.restart()
    ...
    def restart(self):
        ...
        self.camera_manager = CameraManager(self.player, self.hud, self.camera_config)
        self.camera_manager.transform_index = cam_index
    ...
    def reattach_hero(self):
        actors = self.world.get_actors().filter(self._actor_filter)
        for actor in actors:
            if actor.attributes["role_name"] == self.actor_role_name:
                print("Attach actor")
                self.player = actor
                break
...
def main():
    ...
    argparser.add_argument(
        "--attach_ego_vehicle",
        action="store_true",
        help="Do not spawn a new ego vehicle but attach to the existing one",
    )
    argparser.add_argument(
        "--res",
    ...
    argparser.add_argument(
        ...
        default=os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "config/6_camera_setup.ini"
        ),
        type=str,
