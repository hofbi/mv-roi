...
class SplitImagesTest(unittest.TestCase):
    ...
    def test_split_json_data__no_files__empty(self):
        result = split.split_json_data([], None, ".png")
        self.assertFalse(result)

    @patch("builtins.open", new_callable=mock_open, read_data="data")
    @patch("json.loads", MagicMock())
    @patch("annotation.split.crop_from_json", MagicMock())
    def test_split_json_data__two_files_and_test_layout__size_is_12(self, _):
        result = split.split_json_data(
            ["merged_000.json", "merged_001.json"], self.TEST_LAYOUT, ".png"
        )
    ...
    @patch("json.loads", MagicMock())
    @patch("annotation.split.crop_from_json", MagicMock())
    def test_split_json_data__two_files_and_test_layout__correct_file_names(self, _):
        result = split.split_json_data(
            ["merged_000.json", "merged_001.json"], self.TEST_LAYOUT, ".png"
        )
